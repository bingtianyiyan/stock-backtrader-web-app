...
from core.api.kdata import default_adjust_type, get_kdata_schema, get_latest_kdata_date, get_recent_trade_dates
from core.contract import IntervalLevel, AdjustType
from core.contract.api import get_entity_ids
from core.domain import DragonAndTiger, Stock1dHfqKdata, Stock, LimitUpInfo, StockQuote, StockQuoteLog
from core.utils.pd_utils import pd_is_not_null
from core.utils.time_utils import to_pd_timestamp, date_time_by_interval, current_date, now_timestamp
...
def get_shoot_today(up_change_pct=0.03, down_change_pct=-0.03, interval=2):
    ...
    # 双重检查
    if latest.empty or 'time' not in latest.columns:
        latest_time = current_time  # 默认使用当前时间
        print("警告：未获取到有效时间，使用当前时间替代")
    else:
